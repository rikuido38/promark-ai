...
interface MediaItem {
  ...
  type: MediaType
  storagePath?: string     // bucket path — used for server-side copy-to-drafts
  seed_details?: string    // auto-generated image description (scene, style, characters,
                           // brand colors, reference images used) — passed back to the
                           // inner illustration agent on edit requests so it understands
                           // the previous image without needing conversation history
}
...
| `medias` | Ordered list of assets. `signedUrl` must be a valid, time-limited pre-signed URL for private storage assets, or a stable public URL. `seed_details` is auto-generated by the illustration pipeline and persisted in MongoDB alongside the storage path. |
...
Conversation history is persisted in **MongoDB** via the LangGraph `MongoDBSaver` checkpointer (see `lib/agents/checkpointer.ts`), keyed by `sessionId` (`thread_id`). This allows the Main Orchestrator Agent to access full multi-turn history on every request, including across serverless cold starts.

Inner illustration agents (created fresh per request in `buildIllustrationAgent`) are intentionally stateless — they receive all necessary context (previous image URL via `sampleImageUrls`, previous image description via `seed_details`) explicitly from the outer tool call rather than via a checkpointer.
